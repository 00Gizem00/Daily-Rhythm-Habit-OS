...
final class NotificationSettingsModel: ObservableObject {
    ...
    func refresh() async {
        ...
        do {
            let coordinator = try RhythmNotifications.coordinator()
    ...
    func update(_ change: RhythmNotificationPreferenceChange, enabling: Bool = false) async {
        guard !updating else { return }
        updating = true
        refreshID = UUID()
        ...
        do {
            ...
            apply(try await RhythmNotifications.coordinator().update(change))
        } catch {
        ...
        Form {
            Section {
                Toggle("Timed step reminders", isOn: Binding(
                    ...
                    set: { enabled in Task { await notifications.update(.timedSteps(enabled), enabling: enabled) } }))
            } footer: {
            ...
            Section {
                Toggle("Daily Close", isOn: Binding(
                    ...
                    set: { enabled in Task { await notifications.update(.dailyClose(enabled), enabling: enabled) } }))
                if notifications.preferences.dailyClose {
                    DatePicker("Review time", selection: Binding(get: { closeTime }, set: { value in
                        ...
                        Task { await notifications.update(.closeTime(hour: components.hour ?? 20, minute: components.minute ?? 30)) }
