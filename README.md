...
- **Optional pilot diagnostics:** off by default, a 30-day local observation counts first saved completions and categorized action failures. Users choose whether to export aggregate JSON; no habit content or local IDs enter that export, and nothing uploads automatically. See [diagnostics verification](docs/verification/ISSUE-20-PILOT-DIAGNOSTICS-VALIDATION.md) for definitions, 148 passing core tests, 27 native diagnostics checks and remaining UI checks.
