...
These tests prove system-dispatched create/query/complete/repeat/reopen and Spotlight integration on an unlocked phone. They do not prove natural-language routing or locked-device authentication. The user has separately confirmed real Siri creation and completion; see the current [device evidence](verification/ISSUE-10-SCHEMA-VALIDATION.md) for completion and crash-investigation status.
